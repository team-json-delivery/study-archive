...
> * user2 : r1,r3,r5,r6...
> * user3 : r1,r2,r4...
> * 이라고 할때 r2에 교통사고 발생시 r2를 가지고 있는 모든 사용자를 full search 해야 함
> * O(n x m)

> 검색 속도를 높일 수 있는 다른 방법
> * 출발지부터 목적지까지를 모두 포함한 형태로 경로를 보관한다
> * r1, super(r1), super(super(r1))...
> * 가장 큰 map에 교통 상황이 변한 타일이 속한다면 해당 사용자는 영향 받을 가능성이 있다고 볼 수 있다
> * O(n) : 사용자의 경로 목록을 탐색하지 않아도 되기 때문에 m은 없다

* 그러나 도로 상황이 개선되었을 때 다시 그 경로로 돌아갈 수 있도록 할 수 없다
* ETA를 주기적으로 재계산하여 더 짧은 ETA를 갖는 경로가 발견되면 알린다
  * tmap의 경우 약 15분마다 재계산

### 전송 프로토콜
* 경로 안내 중 상황이 변경될 경우가 있으므로
* push / long polling / websocket/ SSE 등을 사용하여 알릴 수 있다
* push는 전송량이 제한적
* long polling 은 서버의 부담
  * 커넥션 유지해야 하는 부담이 있음
> push의 전송이 long polling 을 이용하여 전송하고 있다고 알고 있는데...?
* SSE와 websocket 중에서는 양방향 통신을 지원하는 websocket이 더 좋아보인다
* 목적지에 가까워졌을때 실시간 양방향 통신이 필요한 경우도 있기 때문
  * 도착했다는 정보를 서버에 알리기 위함일듯


![img_4.png](img_4.png)
> 최종.final_(3)
## 4단계: 마무리
* 시스템의 확장에 관심이 있다면 중간 경유지 설정 기능을 고민해 보는 것도 좋ㅇ다
* 이런 기능은 도어대시나 우버, 리프트와 같은 서비스에 아주 유용할 것이다